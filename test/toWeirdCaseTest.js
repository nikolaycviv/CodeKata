...
const toWeirdCase = require('../kata/toWeirdCase').toWeirdCase;
...
describe('toWeirdCase', () => {
    ...
    describe('should return the correct value for a single word', () => {
        ...
        it('LoOkS LiKe YoU PaSsEd', () => {
            assert.equal(toWeirdCase('Looks like you passed'), 'LoOkS LiKe YoU PaSsEd');
        });
        it('ThIs Is ThE FiNaL TeSt CaSe', () => {
            assert.equal(toWeirdCase('This is the final test case'), 'ThIs Is ThE FiNaL TeSt CaSe');
        });
        it('JuSt KiDdInG', () => {
            assert.equal(toWeirdCase('Just kidding'), 'JuSt KiDdInG');
        });
        it('Ok FiNe YoU ArE DoNe NoW', () => {
            assert.equal(toWeirdCase('Ok fine you are done now'), 'Ok FiNe YoU ArE DoNe NoW');
        });
    });
});
